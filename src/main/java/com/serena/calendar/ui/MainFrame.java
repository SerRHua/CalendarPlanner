...
public final class MainFrame extends JFrame {
    ...
    private void renderCalendar() {
        ...
        for(DayOfWeek day:new DayOfWeek[]{DayOfWeek.SUNDAY,DayOfWeek.MONDAY,DayOfWeek.TUESDAY,DayOfWeek.WEDNESDAY,DayOfWeek.THURSDAY,DayOfWeek.FRIDAY,DayOfWeek.SATURDAY}){JLabel l=label(day.getDisplayName(TextStyle.SHORT,Locale.getDefault()).toUpperCase(),11,Font.BOLD,MUTED);l.setHorizontalAlignment(SwingConstants.CENTER);calendarGrid.add(l);}
        LocalDate first=displayedMonth.atDay(1);int offset=first.getDayOfWeek().getValue()%7;for(int i=0;i<offset;i++)calendarGrid.add(new JLabel());for(int d=1;d<=displayedMonth.lengthOfMonth();d++){LocalDate date=displayedMonth.atDay(d);calendarGrid.add(dayButton(date,moneyByDay.getOrDefault(date,new DailyMoney(date,0,0))));}for(int i=offset+displayedMonth.lengthOfMonth();i<42;i++)calendarGrid.add(new JLabel());calendarGrid.revalidate();calendarGrid.repaint();
    }
