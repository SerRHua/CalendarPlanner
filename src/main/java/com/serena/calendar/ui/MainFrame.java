...
public final class MainFrame extends JFrame {
    ...
    private JComponent buildApp() {
        ...
        JButton today = outlineButton("Go to today"); today.addActionListener(e -> { selectedDate=LocalDate.now(); displayedMonth=YearMonth.now(); refreshAll(); }); heading.add(today, BorderLayout.EAST);
    ...
    private JComponent calendarCard() {
        ...
        JButton previous=outlineButton("‹"), next=outlineButton("›"); previous.addActionListener(e->{displayedMonth=displayedMonth.minusMonths(1);renderCalendar();}); next.addActionListener(e->{displayedMonth=displayedMonth.plusMonths(1);renderCalendar();}); nav.add(previous,BorderLayout.WEST); JPanel center=stack(monthTitle,clockLabel,3);monthTitle.setHorizontalAlignment(SwingConstants.CENTER);clockLabel.setHorizontalAlignment(SwingConstants.CENTER);nav.add(center,BorderLayout.CENTER); nav.add(next,BorderLayout.EAST); p.add(nav,BorderLayout.NORTH); calendarGrid.setOpaque(false); p.add(calendarGrid,BorderLayout.CENTER); return p;
    }

    private JComponent tasksPage() {
        RoundedPanel p=card(new BorderLayout(0,14)); JPanel top=new JPanel(new BorderLayout()); top.setOpaque(false); top.add(label("Tasks",18,Font.BOLD,INK),BorderLayout.WEST);
    ...
    private JComponent financePage() {
        JPanel p=new JPanel(new BorderLayout(0,16));p.setOpaque(false); JPanel top=new JPanel(new BorderLayout());top.setOpaque(false);top.add(financeDateLabel,BorderLayout.WEST);p.add(top,BorderLayout.NORTH);
        ...
        JPanel periods=new JPanel(new GridLayout(1,3,12,0));periods.setOpaque(false);periods.setPreferredSize(new Dimension(0,88));periods.add(metricCard("Selected day",dayTotal,CARD));periods.add(metricCard("Selected week",weekTotal,CARD));periods.add(metricCard("Selected month",monthTotal,CARD));
    ...
    private JComponent trendsPage() {
        RoundedPanel p=card(new BorderLayout(0,14));JPanel top=new JPanel(new BorderLayout());top.setOpaque(false);top.add(trendMonthLabel,BorderLayout.WEST);
    ...
    private void refreshAll(){String date=selectedDate.format(FULL_DATE);selectedDateLabel.setText(date);financeDateLabel.setText("Transactions for "+date);trendMonthLabel.setText("Money trends · "+selectedDate.format(DateTimeFormatter.ofPattern("MMMM yyyy")));renderCalendar();load(eventModel,()->db.eventsOn(selectedDate));load(todoModel,db::todos);refreshMoney();}
    private void refreshMoney(){load(expenseModel,()->db.expensesOn(selectedDate));load(incomeModel,()->db.incomeOn(selectedDate));try{long spent=db.overallTotal(),earned=db.overallEarned(),saved=earned-spent;dayTotal.setText(Money.format(db.dayTotal(selectedDate)));weekTotal.setText(Money.format(db.weekTotal(selectedDate)));monthTotal.setText(Money.format(db.monthTotal(selectedDate)));overallSpent.setText(Money.format(spent));overallEarned.setText(Money.format(earned));overallSaved.setText(Money.format(saved));overallSaved.setForeground(saved>=0?GREEN:RED);refreshTrend();}catch(SQLException e){showError(e);}}
    private void refreshTrend(){try{LocalDate start,end;boolean monthly=false;switch(trendRange){case WEEK->{start=selectedDate.minusDays(selectedDate.getDayOfWeek().getValue()-1L);end=start.plusDays(6);}case MONTH->{start=selectedDate.withDayOfMonth(1);end=selectedDate.withDayOfMonth(selectedDate.lengthOfMonth());}case YEAR->{start=selectedDate.minusYears(1).plusDays(1);end=selectedDate;monthly=true;}case THREE_YEARS->{start=selectedDate.minusYears(3).plusDays(1);end=selectedDate;monthly=true;}default->throw new IllegalStateException();}trendMonthLabel.setText("Cumulative money trend · "+trendRange.label+" · "+start.format(DateTimeFormatter.ofPattern("MMM d, yyyy"))+" – "+end.format(DateTimeFormatter.ofPattern("MMM d, yyyy")));trendChart.setData(db.moneyTrend(start,end),monthly);}catch(SQLException e){showError(e);}}
    ...
    private void addExpense(){moneyForm("Add expense","Description",(description,cents)->db.addExpense(selectedDate,description,cents));}
    private void addIncome(){moneyForm("Add money earned","Source (salary, interest, etc.)",(description,cents)->db.addIncome(selectedDate,description,cents));}
    ...
    private static final class ExpenseRenderer extends PaddedRenderer{ExpenseRenderer(){super(INK);}}
    private static final class IncomeRenderer extends PaddedRenderer{IncomeRenderer(){super(GREEN);}}
