...
public final class MainFrame extends JFrame {
    ...
    private JComponent trendsPage() {
        RoundedPanel p=card(new BorderLayout(0,14));JPanel top=new JPanel(new BorderLayout());top.setOpaque(false);top.add(trendMonthLabel,BorderLayout.WEST);top.add(label("Daily totals",12,Font.PLAIN,MUTED),BorderLayout.EAST);p.add(top,BorderLayout.NORTH);p.add(trendChart,BorderLayout.CENTER);return p;
    }
    ...
    private void refreshMoney(){load(expenseModel,()->db.expensesOn(selectedDate));load(incomeModel,()->db.incomeOn(selectedDate));try{long spent=db.overallTotal(),earned=db.overallEarned(),saved=earned-spent;dayTotal.setText(Money.format(db.dayTotal(selectedDate)));weekTotal.setText(Money.format(db.weekTotal(selectedDate)));monthTotal.setText(Money.format(db.monthTotal(selectedDate)));overallSpent.setText(Money.format(spent));overallEarned.setText(Money.format(earned));overallSaved.setText(Money.format(saved));overallSaved.setForeground(saved>=0?GREEN:RED);trendChart.setData(db.moneyTrend(YearMonth.from(selectedDate)));}catch(SQLException e){showError(e);}}
